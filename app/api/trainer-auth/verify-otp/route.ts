...
export async function POST(req: NextRequest) {
  ...
  const { data: trainer } = await supabase.from("trainers").select("id").eq("phone", phone).maybeSingle();
  if (!trainer) {
    return NextResponse.json({ ok: false, error: "This number isn't registered as a trainer." }, { status: 404 });
  }
  ...
  const { error: sessionError } = await supabase
    ...
    .insert({ trainer_id: trainer.id, token, expires_at: expiresAt });
  if (sessionError) {
